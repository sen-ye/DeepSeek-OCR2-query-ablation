...
class DeepseekOCR2Model(DeepseekV2Model):
    ...
    def __init__(self, config: DeepseekV2Config):
        ...
        # Optional ablation controls (default: disabled). These are used only by
        # the ablation scripts and do not affect normal inference/training.
        self._causal_flow_ablation = None
        self._causal_flow_ablation_donor_tokens = None

    def set_causal_flow_ablation(self, ablation: Optional[dict] = None, donor_tokens: Optional[torch.Tensor] = None):
        self._causal_flow_ablation = ablation
        self._causal_flow_ablation_donor_tokens = donor_tokens

    def _apply_causal_flow_ablation(self, tokens: torch.Tensor) -> torch.Tensor:
        cfg = getattr(self, "_causal_flow_ablation", None)
        if not cfg:
            return tokens
        ...
        _, seq_len, _ = tokens_3d.shape
        if seq_len <= 1:
            return tokens

        if mode == "replace_second_half":
            print(f"replacing second half with first half")
            half = seq_len // 2
            out = tokens_3d.clone()
            out[:, half:, :] = out[:, : seq_len - half, :]
        elif mode == "swap_halves":
            print(f"swapping halves")
            half = seq_len // 2
            out = torch.cat([tokens_3d[:, half:, :], tokens_3d[:, :half, :]], dim=1)
        elif mode == "shuffle":
            print(f"shuffling")
            seed = cfg.get("seed", None)
            g = torch.Generator(device=tokens_3d.device)
            if seed is not None:
                g.manual_seed(int(seed))
            perm = torch.randperm(seq_len, generator=g, device=tokens_3d.device)
            out = tokens_3d.index_select(1, perm)
        elif mode == "cross_image_replace":
            print(f"replacing with donor tokens")
            donor = getattr(self, "_causal_flow_ablation_donor_tokens", None)
            if donor is None:
                return tokens
            if donor.dim() == 2:
                donor = donor.unsqueeze(0)
            if donor.shape[1] != seq_len:
                raise ValueError(f"donor_tokens length mismatch: donor={donor.shape[1]} vs tokens={seq_len}")
            half = seq_len // 2
            out = tokens_3d.clone()
            out[:, half:, :] = donor[:, : seq_len - half, :]
        else:
            print(f"no mode selected, returning original tokens")
            return tokens
    ...
    @torch.no_grad()
    def encode_global_view(self, image_tensor: torch.Tensor, *, return_attentions: bool = False):
        """
        Encode a single global-view image tensor to causal flow tokens (after projector).
        `image_tensor`: [B, 3, H, W]
        """
        global_features_1 = self.sam_model(image_tensor)
        if return_attentions:
            global_features_2, attentions = self.qwen2_model(global_features_1, return_attentions=True)
            global_features = self.projector(global_features_2)
            return global_features, attentions
            ...
            for image, crop_shape in zip(images, images_spatial_crop):
                ...
                # with torch.inference_mode(): 
                    ...
                    if torch.sum(patches).item() != 0:
                        ...
                        global_features = self._apply_causal_flow_ablation(global_features)
...
class DeepseekOCR2ForCausalLM(DeepseekV2ForCausalLM):
    ...
    def infer(
        ...
        if not eval_mode:
            ...
            no_repeat_ngram_size = no_repeat_ngram_size if no_repeat_ngram_size is not None else 20
            ...
            no_repeat_ngram_size = no_repeat_ngram_size if no_repeat_ngram_size is not None else 35
            with torch.autocast("cuda", dtype=torch.bfloat16):
                with torch.no_grad():
                    output_ids = self.generate(
                        input_ids.unsqueeze(0).cuda(),
